...
# ── Constants ────────────────────────────────────────────────────────────
_MEMORIAL_PREFIX = "MEM-"
_WARNING_COLOR = "#b05000"
_SPLIT_SUFFIX_RE = re.compile(r"-\d+$")
_MEMORIAL_LEG_RE = re.compile(r"-[DC]$")
...
class RemoveCategoryDialog(QDialog):
    ...
    def _reference_is_memorial(reference: Optional[str]) -> bool:
        return (reference or "").startswith(_MEMORIAL_PREFIX)

    @staticmethod
    def _memorial_base_ref(reference: Optional[str]) -> str:
        """Strip the trailing -D or -C leg suffix."""
        return _MEMORIAL_LEG_RE.sub("", reference or "")
